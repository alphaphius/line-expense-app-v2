...
test('reports support reusable templates, site-scoped equipment IDs, CSV imports, favorites, and hierarchical export selection', async () => {
  ...
  assert.match(reports, /workhub-installation-report-favorites-v1/);
  assert.match(reports, /localStorage\.setItem\(FAVORITES_KEY/);
  assert.match(reports, /state\.favorites\.groupId/);
  assert.doesNotMatch(reports, /state\.data\.favorites\.(?:groupId|siteIds)/);
  ...
  assert.match(reports, /report-module-heading__title/);
  assert.match(styles, /\.report-module-heading__title/);
