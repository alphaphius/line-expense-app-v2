...
app.get('/api/health',async()=>({ok:true,appName:config.appName,version:config.appVersion,database:'connected',time:new Date().toISOString()}));
await app.register(fastifyStatic,{root:config.publicDir,prefix:'/',maxAge:'1h',immutable:false,index:['index.html'],setHeaders(reply,filePath){if(filePath.endsWith('index.html')||filePath.endsWith('config.js')||filePath.endsWith('service-worker.js'))reply.header('Cache-Control','no-cache');else if(/\.(js|css|wasm|gz)$/.test(filePath))reply.header('Cache-Control','public, max-age=31536000, immutable');}});
