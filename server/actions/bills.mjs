...
export async function exportMonthlyBillExcel(period,createTicket){const{month,rows}=await monthlyBills(period);const headers=['ลำดับ','วันที่เอกสาร','เลขที่เอกสาร','ผู้ขาย','เลขผู้เสียภาษีผู้ขาย','โครงการ','บริษัท','หมวดหมู่','ยอดก่อนภาษี','VAT','หัก ณ ที่จ่าย','ยอดสุทธิ','สถานะ'];const values=rows.map((row,index)=>[index+1,row.document_date||'',row.document_no||'',row.vendor_name||'',row.vendor_tax_id||'',row.project_name||'',row.company_name||'',row.category_name||'',number(row.subtotal),number(row.vat_amount),number(row.withholding_tax),number(row.grand_total),row.status]);const buffer=await createXlsx('รายการบิล',headers,values,[8,14,20,30,18,28,32,24,16,14,16,16,18]);return createTicket(buffer,`สรุปบิล_${month}.xlsx`,XLSX_MIME,rows.length);}
