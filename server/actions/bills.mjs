...
export async function getBillDetail(billId){const row=await one("SELECT b.*,p.project_name,co.company_name,c.category_name,COALESCE(NULLIF(lu.workhub_name, ''), NULLIF(b.source_user_name, ''), NULLIF(lu.display_name, ''), IF(b.source='LINE','ผู้ส่งผ่าน LINE','เว็บแอป')) AS owner_display_name FROM bills b LEFT JOIN projects p ON p.project_id=b.project_id LEFT JOIN companies co ON co.company_id=b.company_id LEFT JOIN categories c ON c.category_id=b.category_id LEFT JOIN line_users lu ON lu.user_id=b.source_user_id WHERE b.bill_id=:id",{id:clean(billId,64)});if(!row)throw apiError('BILL_NOT_FOUND','ไม่พบบิล');const [items,documents]=await Promise.all([select('SELECT * FROM bill_items WHERE bill_id=:id ORDER BY line_no',{id:billId}),select('SELECT doc_id,bill_id,session_id,page_no,file_name,mime_type,sha256,size_bytes,original_size_bytes,compression,width,height,created_at FROM bill_documents WHERE bill_id=:id ORDER BY page_no',{id:billId})]);return{...enrichBill(row),items:items.map(publicRow),documents:documents.map(publicRow)};}
