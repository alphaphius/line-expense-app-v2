(function () {
  ...
  function renderReportSites() {
    ...
    document.getElementById('report-content').innerHTML=`<section class="module-panel report-journey"><div class="module-toolbar report-sites-toolbar"><h4>${esc(group.name)}</h4><button class="module-secondary" data-csv-import="sites" aria-label="นำเข้าไซต์จาก CSV" title="นำเข้าไซต์ CSV">นำเข้าไซต์ CSV</button></div>${reportCrumbs([{label:'กลุ่มงาน',action:'groups'},{label:group.name}])}<div class="report-province-list">${sections||'<div class="module-empty"><strong>ยังไม่มีไซต์ในกลุ่มงานนี้</strong><p>เพิ่มไซต์หรือนำเข้าจาก CSV ก่อนดำเนินการต่อ</p></div>'}</div></section>`;
  }
