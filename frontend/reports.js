(function () {
  ...
  function seed() {
    return {
      ...
      sites:[
        {uid:'site-a',id:'SITE-A',groupId:'wg-2569',name:'ไซต์ A โรงไฟฟ้าบางปะกง',customer:'บริษัทตัวอย่าง A',province:'ฉะเชิงเทรา',address:'จังหวัดฉะเชิงเทรา'},
        {uid:'site-b',id:'SITE-B',groupId:'wg-2569',name:'ไซต์ B คลังน้ำมันศรีราชา',customer:'บริษัทตัวอย่าง B',province:'ชลบุรี',address:'จังหวัดชลบุรี'},
        {uid:'site-c',id:'SITE-C',groupId:'wg-2569',name:'ไซต์ C โรงงานอาหารสมุทรสาคร',customer:'บริษัทตัวอย่าง C',province:'สมุทรสาคร',address:'จังหวัดสมุทรสาคร'},
      ],
      equipment:[
  ...
  function load() {
    ...
    if (!state.data.groups.some(group=>group.id===state.data.favorites.groupId)) state.data.favorites.groupId='';
    state.data.favorites.siteIds=state.data.favorites.siteIds.filter(id=>state.data.sites.some(site=>site.uid===id));
    state.data.sites.forEach(site=>{site.province=site.province||provinceFromAddress(site.address);});
    save();
  }
  ...
  function shell() {
    root().innerHTML=`<div class="module-shell report-shell">
      ...
      <div class="report-tabs-wrap"><nav class="module-tabs report-tabs" aria-label="เมนูรายงาน">
        <button data-report-tab="reports">กรอกข้อมูลและ Export</button><button data-report-tab="templates">Templates</button><button data-report-tab="structure">กลุ่มงานและไซต์</button><button data-report-tab="assignments">อุปกรณ์และ Template</button><button data-report-tab="overview">ภาพรวม</button>
    ...
    const tabs=root().querySelector('.report-tabs');tabs.addEventListener('scroll',updateReportTabCue,{passive:true});requestAnimationFrame(updateReportTabCue);
    bind(); render();
  }
  ...
  function renderReportGroups() {
    const favoriteId=state.data.favorites.groupId;
    const cards=state.data.groups.map(group=>{const sites=state.data.sites.filter(site=>site.groupId===group.id),equipment=state.data.equipment.filter(item=>item.groupId===group.id),ready=equipment.filter(item=>progress(item).status==='READY').length,percent=equipment.length?Math.round(ready/equipment.length*100):0,isFavorite=group.id===favoriteId;return `<article class="report-group-card ${isFavorite?'is-favorite':''}"><button class="report-favorite-button" data-favorite-group="${esc(group.id)}" aria-pressed="${isFavorite}" aria-label="${isFavorite?'ยกเลิกกลุ่มโปรด':'ตั้งเป็นกลุ่มโปรด'}" title="${isFavorite?'ยกเลิกกลุ่มโปรด':'เข้ากลุ่มนี้ทันทีเมื่อเปิดรายงาน'}">${isFavorite?'♥':'♡'}</button><button class="report-group-card__body" data-report-group-open="${esc(group.id)}"><span class="report-group-card__icon">${esc(String(group.fiscalYear||group.code).slice(-2))}</span><span><small>กลุ่มงาน</small><strong>${esc(group.name)}</strong><em>${esc(group.code)} · ${sites.length} ไซต์ · ${equipment.length} อุปกรณ์</em></span><span class="report-group-card__progress"><b>${percent}%</b><i><u style="width:${percent}%"></u></i><small>${ready} พร้อม Export</small></span><span class="report-card-arrow">›</span></button></article>`;}).join('');
    document.getElementById('report-content').innerHTML=`<section class="module-panel report-journey"><div class="module-toolbar"><div><h4>เลือกกลุ่มงาน</h4><p>แตะหัวใจเพื่อให้ครั้งถัดไปเปิดเข้าหน้าไซต์ของกลุ่มนี้ทันที</p></div><button class="module-secondary" data-csv-import="groups" aria-label="นำเข้ากลุ่มงานจาก CSV" title="นำเข้ากลุ่มงาน CSV">นำเข้ากลุ่มงาน CSV</button></div>${reportCrumbs([{label:'กลุ่มงาน'}])}<div class="report-group-grid">${cards||'<div class="module-empty"><strong>ยังไม่มีกลุ่มงาน</strong><p>สร้างกลุ่มงานหรือนำเข้าจาก CSV ก่อนเริ่มกรอกข้อมูล</p></div>'}</div></section>`;
  }
  function renderReportSites() {
    const group=selectedGroup(),sites=state.data.sites.filter(site=>site.groupId===group.id),favoriteIds=new Set(state.data.favorites.siteIds),favoriteSites=sites.filter(site=>favoriteIds.has(site.uid)),regularSites=sites.filter(site=>!favoriteIds.has(site.uid)),byProvince=regularSites.reduce((out,site)=>{const province=site.province||provinceFromAddress(site.address);(out[province]||(out[province]=[])).push(site);return out;},{});
    const siteCard=site=>{const equipment=state.data.equipment.filter(item=>item.siteId===site.uid),ready=equipment.filter(item=>progress(item).status==='READY').length,types=new Set(equipment.map(item=>item.type)).size,isFavorite=favoriteIds.has(site.uid);return `<article class="report-journey-site ${isFavorite?'is-favorite':''}"><button class="report-favorite-button" data-favorite-site="${esc(site.uid)}" aria-pressed="${isFavorite}" aria-label="${isFavorite?'ยกเลิกไซต์โปรด':'เพิ่มไซต์โปรด'}" title="${isFavorite?'ยกเลิกไซต์โปรด':'แสดงไซต์นี้ในหมวด Favorite'}">${isFavorite?'♥':'♡'}</button><button class="report-journey-site__body" data-report-site-open="${esc(site.uid)}"><span><small>${esc(site.id)}</small><strong>${esc(site.name)}</strong><em>${esc(site.customer||'ไม่ระบุลูกค้า')}</em></span><span class="report-site-counts"><b>${equipment.length}</b> อุปกรณ์ · ${types} ประเภท<br><i>${ready} พร้อม Export</i></span><span class="report-card-arrow">›</span></button></article>`;};
    const favoriteSection=favoriteSites.length?`<details class="report-province report-province--favorites" open><summary><span>♥</span><div><small>ทางลัด</small><h5>Favorite</h5></div><strong>${favoriteSites.length} ไซต์</strong></summary><div class="report-site-grid">${favoriteSites.map(siteCard).join('')}</div></details>`:'';
    const provinceSections=Object.entries(byProvince).sort(([a],[b])=>a.localeCompare(b,'th')).map(([province,rows])=>`<details class="report-province" open><summary><span>⌖</span><div><small>จังหวัด</small><h5>${esc(province)}</h5></div><strong>${rows.length} ไซต์</strong></summary><div class="report-site-grid">${rows.map(siteCard).join('')}</div></details>`).join('');
    const sections=favoriteSection+provinceSections;
    document.getElementById('report-content').innerHTML=`<section class="module-panel report-journey"><div class="module-toolbar"><div><h4>${esc(group.name)}</h4><p>กดชื่อจังหวัดเพื่อพับหรือขยาย และแตะหัวใจให้ไซต์ที่ใช้บ่อยอยู่บนสุด</p></div><button class="module-secondary" data-csv-import="sites" aria-label="นำเข้าไซต์จาก CSV" title="นำเข้าไซต์ CSV">นำเข้าไซต์ CSV</button></div>${reportCrumbs([{label:'กลุ่มงาน',action:'groups'},{label:group.name}])}<div class="report-province-list">${sections||'<div class="module-empty"><strong>ยังไม่มีไซต์ในกลุ่มงานนี้</strong><p>เพิ่มไซต์หรือนำเข้าจาก CSV ก่อนดำเนินการต่อ</p></div>'}</div></section>`;
  }
  function renderReportEquipment() {
    const group=selectedGroup(),site=state.data.sites.find(row=>row.uid===state.selectedSite);if(!site){state.reportStep='sites';return renderReportSites();}const equipment=state.data.equipment.filter(item=>item.siteId===site.uid),groups=equipment.reduce((out,item)=>((out[item.type]||(out[item.type]=[])).push(item),out),{});
    const lists=Object.entries(groups).sort(([a],[b])=>a.localeCompare(b)).map(([type,items])=>{const template=templateForEquipment(items[0]);return `<section class="report-device-group"><header><label><input type="checkbox" data-report-type="${esc(type)}" data-report-type-site="${esc(site.uid)}" ${items.length&&items.every(item=>state.selectedReports.has(item.uid))?'checked':''}><span><strong>${esc(type)}</strong><small>${template?esc(template.name):'ยังไม่ได้กำหนด Template'}</small></span></label><div><b>${items.length} ตัว</b>${template?`<button data-report-csv-template="${esc(template.id)}">CSV</button>`:''}</div></header><div>${items.map(item=>{const p=progress(item),tpl=templateForEquipment(item);return `<article class="report-device-row"><input type="checkbox" data-report-check="${esc(item.uid)}" ${state.selectedReports.has(item.uid)?'checked':''}><button data-report-open="${esc(item.uid)}"><span class="report-device-id">${esc(item.id)}</span><span><strong>${esc(item.name)}</strong><small>S/N ${esc(item.serial||'-')} · ${esc(item.location||'ไม่ระบุตำแหน่ง')}</small><em>${esc(tpl?.name||'ยังไม่มี Template')}</em></span><span class="report-device-progress"><b>${p.percent}%</b><i><u style="width:${p.percent}%"></u></i></span>${statusPill(p.status)}<span class="report-card-arrow">›</span></button></article>`;}).join('')}</div></section>`;}).join('');
    document.getElementById('report-content').innerHTML=`<section class="module-panel report-journey report-equipment-page"><div class="module-toolbar"><div><h4>${esc(site.name)}</h4><p>${esc(site.id)} · ${esc(site.province||provinceFromAddress(site.address))} · แสดงอุปกรณ์ทั้งหมดโดยไม่ต้องเลือก Filter</p></div><button class="module-secondary" data-csv-import="equipment" aria-label="นำเข้าอุปกรณ์จาก CSV" title="นำเข้าอุปกรณ์ CSV">นำเข้าอุปกรณ์ CSV</button></div>${reportCrumbs([{label:'กลุ่มงาน',action:'groups'},{label:group.name,action:'sites'},{label:site.name}])}<div class="report-equipment-summary"><span><strong>${equipment.length}</strong> อุปกรณ์</span><span><strong>${Object.keys(groups).length}</strong> ประเภท</span><button class="module-secondary" data-select-ready>เลือกทั้งหมดที่พร้อม</button></div><div class="report-device-groups">${lists||'<div class="module-empty"><strong>ไซต์นี้ยังไม่มีอุปกรณ์</strong><p>เพิ่มอุปกรณ์หรือนำเข้าจาก CSV แล้วรายการทั้งหมดจะแสดงที่นี่</p></div>'}</div>${reportSelectionBar(equipment)}</section>`;
  }
  ...
  function simpleEntityDialog(entity) {
    ...
    showDialog(`<form data-entity-form="${entity}" class="wh-form"><div class="wh-dialog__head"><div><span>INSTALLATION REPORTS</span><h4>${spec[0]}</h4></div><button value="cancel">×</button></div>${entity!=='group'?`<label>กลุ่มงาน<select name="groupId" required>${optionRows(state.data.groups,'id','name',group.id)}</select></label>`:''}${entity==='equipment'?`<label>ไซต์งาน<select name="siteId" required>${optionRows(state.data.sites.filter(item=>item.groupId===group.id),'uid','name',site?.uid)}</select></label>`:''}<label>${entity==='group'?'รหัสกลุ่มงาน':entity==='site'?'Site ID':'Equipment ID'}<input name="code" required placeholder="${entity==='group'?'FY2570':entity==='site'?'SITE-D':'SM1'}"></label><label>${spec[1]}<input name="name" required placeholder="${spec[2]}"></label>${entity==='group'?`<label>ปีงบประมาณ<input name="fiscalYear" placeholder="2570"></label>`:''}${entity==='site'?`<div class="form-grid"><label>จังหวัด<input name="province" required placeholder="เช่น ระยอง"></label><label>ลูกค้า<input name="customer"></label></div><label>ที่อยู่<textarea name="address" rows="2"></textarea></label>`:''}${entity==='equipment'?`<div class="form-grid"><label>ประเภทอุปกรณ์<input name="type" required placeholder="Soil Moisture Sensor"></label><label>Model<input name="model"></label></div><div class="form-grid"><label>S/N<input name="serial"></label><label>ตำแหน่งติดตั้ง<input name="location"></label></div>`:''}<div class="wh-form__actions"><button value="cancel" class="module-secondary">ยกเลิก</button><button type="submit" class="module-primary">บันทึก</button></div></form>`);
  }

  const csvSpecs={
    groups:{label:'กลุ่มงาน',headers:['work_group_id','work_group_name','fiscal_year'],sample:[['FY2570','ปีงบประมาณ 2570','2570']]},
    sites:{label:'ไซต์งาน',headers:['work_group_id','site_id','site_name','province','customer','address'],sample:[['FY2569','SITE-D','ไซต์ D โรงงานระยอง','ระยอง','บริษัทตัวอย่าง','จังหวัดระยอง']]},
  ...
  async function importCsv(entity,file,policy) {
    ...
    if(entity==='sites')for(const row of rows){const group=state.data.groups.find(g=>g.code===row.work_group_id);if(!group){conflicts.push(`ไม่พบกลุ่ม ${row.work_group_id}`);continue;}let item=state.data.sites.find(s=>s.groupId===group.id&&s.id===row.site_id);if(!item){state.data.sites.push({uid:uid('site'),id:row.site_id,groupId:group.id,name:row.site_name,province:row.province||provinceFromAddress(row.address),customer:row.customer,address:row.address});created++;}else if(policy!=='skip'){merge(item,{name:row.site_name,province:row.province||provinceFromAddress(row.address),customer:row.customer,address:row.address},['name','province','customer','address']);updated++;}}
  ...
  function bind() {
    ...
    root().addEventListener('click',async event=>{
      const tab=event.target.closest('[data-report-tab]');if(tab){state.active=tab.dataset.reportTab;if(state.active==='reports'&&!state.reportStep)state.reportStep='groups';render();return;}
      ...
      const favoriteGroup=event.target.closest('[data-favorite-group]');if(favoriteGroup){const id=favoriteGroup.dataset.favoriteGroup;state.data.favorites.groupId=state.data.favorites.groupId===id?'':id;persist(state.data.favorites.groupId?'ตั้งเป็นกลุ่มงานโปรดแล้ว':'ยกเลิกกลุ่มงานโปรดแล้ว');return;}
      const favoriteSite=event.target.closest('[data-favorite-site]');if(favoriteSite){const id=favoriteSite.dataset.favoriteSite,index=state.data.favorites.siteIds.indexOf(id);index>=0?state.data.favorites.siteIds.splice(index,1):state.data.favorites.siteIds.unshift(id);persist(index>=0?'ยกเลิกไซต์โปรดแล้ว':'เพิ่มไซต์โปรดแล้ว');return;}
      const groupOpen=event.target.closest('[data-report-group-open]');if(groupOpen){state.selectedGroup=groupOpen.dataset.reportGroupOpen;state.selectedSite='';state.selectedEquipment='';state.reportStep='sites';renderReports();return;}
      ...
      const reportCsv=event.target.closest('[data-report-csv-template]');if(reportCsv)return reportCsvDialog(reportCsv.dataset.reportCsvTemplate);
    ...
    root().addEventListener('submit',async event=>{
      ...
      if(form.matches('[data-entity-form]')){const entity=form.dataset.entityForm;if(entity==='group'){if(state.data.groups.some(g=>g.code===values.code.trim()))return alert('รหัสกลุ่มงานนี้มีอยู่แล้ว');const item={id:uid('wg'),code:values.code.trim(),name:values.name.trim(),fiscalYear:values.fiscalYear.trim()};state.data.groups.push(item);state.selectedGroup=item.id;}if(entity==='site'){const groupId=values.groupId;if(state.data.sites.some(s=>s.groupId===groupId&&s.id===values.code.trim()))return alert('Site ID นี้มีอยู่แล้วในกลุ่มงาน');const item={uid:uid('site'),id:values.code.trim(),groupId,name:values.name.trim(),province:values.province.trim(),customer:values.customer.trim(),address:values.address.trim()};state.data.sites.push(item);state.selectedSite=item.uid;}if(entity==='equipment'){const siteId=values.siteId;if(state.data.equipment.some(e=>e.siteId===siteId&&e.id===values.code.trim()))return alert('Equipment ID นี้มีอยู่แล้วในไซต์');const item={uid:uid('eq'),id:values.code.trim(),groupId:values.groupId,siteId,type:values.type.trim(),name:values.name.trim(),model:values.model.trim(),serial:values.serial.trim(),location:values.location.trim()};state.data.equipment.push(item);state.selectedEquipment=item.uid;}closeDialog();persist('บันทึกข้อมูลแล้ว');return;}
