(function () {
  ...
  function renderReportGroups() {
    const cards=state.data.groups.map(group=>{const sites=state.data.sites.filter(site=>site.groupId===group.id),equipment=state.data.equipment.filter(item=>item.groupId===group.id),ready=equipment.filter(item=>progress(item).status==='READY').length,percent=equipment.length?Math.round(ready/equipment.length*100):0;return `<button class="report-group-card" data-report-group-open="${esc(group.id)}"><span class="report-group-card__icon">${esc(String(group.fiscalYear||group.code).slice(-2))}</span><span><small>กลุ่มงาน</small><strong>${esc(group.name)}</strong><em>${esc(group.code)} · ${sites.length} ไซต์ · ${equipment.length} อุปกรณ์</em></span><span class="report-group-card__progress"><b>${percent}%</b><i><u style="width:${percent}%"></u></i><small>${ready} พร้อม Export</small></span><span class="report-card-arrow">›</span></button>`;}).join('');
    document.getElementById('report-content').innerHTML=`<section class="module-panel report-journey"><div class="module-toolbar"><div><h4>เลือกกลุ่มงาน</h4><p>เริ่มจากกลุ่มงาน แล้วระบบจะแสดงไซต์แยกตามจังหวัด</p></div><button class="module-secondary" data-csv-import="groups">นำเข้ากลุ่มงาน CSV</button></div>${reportCrumbs([{label:'กลุ่มงาน'}])}<div class="report-group-grid">${cards||'<div class="module-empty"><strong>ยังไม่มีกลุ่มงาน</strong><p>สร้างกลุ่มงานหรือนำเข้าจาก CSV ก่อนเริ่มกรอกข้อมูล</p></div>'}</div></section>`;
  }
  function renderReportSites() {
    const group=selectedGroup(),sites=state.data.sites.filter(site=>site.groupId===group.id),byProvince=sites.reduce((out,site)=>{const province=site.province||provinceFromAddress(site.address);(out[province]||(out[province]=[])).push(site);return out;},{});
    const sections=Object.entries(byProvince).sort(([a],[b])=>a.localeCompare(b,'th')).map(([province,rows])=>`<section class="report-province"><header><span>⌖</span><div><small>จังหวัด</small><h5>${esc(province)}</h5></div><strong>${rows.length} ไซต์</strong></header><div class="report-site-grid">${rows.map(site=>{const equipment=state.data.equipment.filter(item=>item.siteId===site.uid),ready=equipment.filter(item=>progress(item).status==='READY').length,types=new Set(equipment.map(item=>item.type)).size;return `<button class="report-journey-site" data-report-site-open="${esc(site.uid)}"><span><small>${esc(site.id)}</small><strong>${esc(site.name)}</strong><em>${esc(site.customer||'ไม่ระบุลูกค้า')}</em></span><span class="report-site-counts"><b>${equipment.length}</b> อุปกรณ์ · ${types} ประเภท<br><i>${ready} พร้อม Export</i></span><span class="report-card-arrow">›</span></button>`;}).join('')}</div></section>`).join('');
    document.getElementById('report-content').innerHTML=`<section class="module-panel report-journey"><div class="module-toolbar"><div><h4>${esc(group.name)}</h4><p>เลือกไซต์เพื่อดูอุปกรณ์ทั้งหมดภายในไซต์นั้น</p></div><button class="module-secondary" data-csv-import="sites">นำเข้าไซต์ CSV</button></div>${reportCrumbs([{label:'กลุ่มงาน',action:'groups'},{label:group.name}])}<div class="report-province-list">${sections||'<div class="module-empty"><strong>ยังไม่มีไซต์ในกลุ่มงานนี้</strong><p>เพิ่มไซต์หรือนำเข้าจาก CSV ก่อนดำเนินการต่อ</p></div>'}</div></section>`;
  }
